...
myCallback = (dataFromChild) => {
  ...
  render() {
    ...
    let _generateOptions = (function (options, _questionNumber, _dataFromChild, callbackFn) {
      return (
        <div className="row">
    ...
    return (
      <div>
        ...
        {_generateOptions(shuffledOptions, this.props.questionNumber,this.listDataFromChild, this.myCallback)}
      </div>
    );
  }
}
...
//
//   $("#ul-"+this.props.questionNumber+" > li").each(function( index ){
//       console.log("ul-" + index + "=>" +    $(this).attr('class').indexOf('selected')  );
//         if ( $(this).attr('class').indexOf('up') === -1 ) {
//             console.log("tgc-" + index + "=>" +    $(this).children().attr('class')  );
//             console.log('***toggling now active***');
//             $($(this).children('div.up')).removeClass('active');
//         }
//   });
//
// }
...
// if ( $(this).attr('class').indexOf('up') === -1 ) {
//     console.log("tgc-" + index + "=>" +    $(this).children().attr('class')  );
//     console.log('***_deselectOptions ***');
//
//     // $($(this).children('div.up')).removeClass('active');
//     // $($(this).children('a')).removeClass('active');
//     // $(this).removeClass('selected up').addClass('unselected');
//       //  $("#ul-2 > #li-2").click();
// }
...
_addOptionUpNoStoring(event) {
  event.preventDefault();
//  console.log(!this._checkClass('up'));

//  if(!this._checkClass('selected up')) {
//        console.log('class up selected')

    if (this.state.cup==0) {
        console.log('add up option')
      this.state.cup=1;
  //    this.setState({ up: false });
      //this.setState({ down: false });
    } else {
      console.log('remove up option')
      this.state.cup=0;
  //    this.setState({ up: false });
    }
...
if( index === -1 || Number(index) ===  Number(this.props.id)  ) {
      ...
      if (this.state.cup==0 ) {
          console.log('ok add option')
        store(this.props.questionNumber, this.props.description, {
  ...
  _updateOptionRendering(event) {
    event.preventDefault();
     (this.state.up == false  && this.state.down == false ) ? this.setState({selected: false}) : this.setState({selected: true}) ;
    //  if (this.state.cup==1 )  {
    //    $("#ul-"+this.props.questionNumber+" > #li-"+this.props.id ).addClass('up');
    //    console.log('adding up class');
    //     console.log($("#ul-"+this.props.questionNumber+" > #li-"+this.props.id ).addClass('up')) ;
    //   }
   }
}
